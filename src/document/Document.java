...
public abstract class Document {
	 ...
	 */
	private Set<Character> syllablesSet;
	
	protected Document(String text)
	{
		this.text = text;
		syllablesSet = new HashSet<Character>();
		syllablesSet.add('a');
		syllablesSet.add('e');
		syllablesSet.add('i');
		syllablesSet.add('o');
	...
	protected int countSyllables(String word)
	{
		...
		while (index < word.length()) {
			if (syllablesSet.contains(Character.toLowerCase(word.charAt(index)))) {
				numSyllabes++;
				index ++;
				while (index < word.length() && syllablesSet.contains(Character.toLowerCase(word.charAt(index)))) {
					index++;
				}
			}else 
				index ++;
		}
		if (word.length() > 0 && Character.toLowerCase(word.charAt(word.length()-1)) == 'e' && numSyllabes > 1 
				&& ! syllablesSet.contains(Character.toLowerCase(word.charAt(word.length()-2)))) numSyllabes--;
	    return numSyllabes;
	}
	...
	public double getFleschScore()
	{
		...
		System.out.println("Number of sentences: " + getNumSentences());
